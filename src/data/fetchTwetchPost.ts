...
export default async function fetchTwetchPost(
  txid: string
): Promise<TwetchPost> {
  const [
    onChainData,
  ...
  return {
    ...
    files: files
      ? await Promise.all(
          (JSON.parse(files) as string[]).map(fetchBitcoinFileMedia)
        )
      : [],
    user: {
      id: Number(id),
...
async function fetchOnChainData(txid: string) {
  ...
  bitcoinFilesMedia =
    bitcoinFilesMedia ||
    (bitcoinFilesUrlMatch
      ? await fetchBitcoinFileMedia(bitcoinFilesUrlMatch[1])
      : undefined);
...
async function parseTwetchOutput(
  ...
  if (type === "text/plain") {
    ...
    if (
      !(
        mapCells[0].s === "1PuQa7K62MiKCtssSLKy1kh56WWU7MtUR5" &&
        mapCells[1].s === "SET"
      )
    )
      return null;
